...
document.addEventListener('DOMContentLoaded', function() {
	...
	async function checkUsername(e) {
		...
		if (!regExp.test(username)) {
			usernameDiv.innerHTML = "아이디: 8 ~ 16 자의 영문 소문자, 숫자와 특수기호(_)만 사용 가능합니다."
			return;
		}
		...
		if (res.data === 'Y') {
			usernameDiv.innerHTML = '아이디: 사용할 수 없는 아이디입니다. 다른 아이디를 입력해 주세요.';
			idChecked = false;
		} else {
	...
	async function checkNickname(e) {
		...
		if (!regExp.test(nickname)) {
			nicknameDiv.innerHTML = '닉네임: 2 ~ 10자의 한글, 영문 대/소문자, 숫자와 특수기호(_)만  사용해 주세요. (공백 사용 불가)'
			return;
		}
		...
		if (res.data === 'Y') {
			nicknameDiv.innerHTML = '닉네임: 사용할 수 없는 닉네임입니다. 다른 닉네임을 입력해 주세요.';
			nickChecked = false;
		} else {
	...
	async function checkEmail(e) {
		...
		if (!regExp.test(email)) {
			emailDiv.innerHTML = '이메일: 이메일 주소가 정확한지 확인해 주세요.';
			return;
		}
		...
		if (res.data === 'Y') {
			emailDiv.innerHTML = '이메일: 가입중인 아이디는 사용하실 수 없습니다.';
			emailChecked = false;
		} else {
			emailDiv.innerHTML = '';
			emailChecked = true;
	...
	function checkPassword(e) {
		...
		if (!regExp.test(password)) {
			passwordDiv.innerHTML = '비밀번호: 8~16자의 영문 대/소문자, 숫자, 특수문자(_!@#$&*)를 사용해 주세요.';
			pwdChecked = false;
			return;
		} else {
